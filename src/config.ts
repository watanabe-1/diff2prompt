...
type StringKeys<T, IncludeOptional extends boolean = true> = KeysByType<T, string, IncludeOptional>;
type NumberKeys<T, IncludeOptional extends boolean = true> = KeysByType<T, number, IncludeOptional>;
...
function pickString(obj: Record<string, unknown>, key: StringKeys<UserConfig>): string | undefined {
  const v = obj[key];
...
export function normalizeUserConfig(cfgRaw: unknown, baseDir: string): Partial<Options> {
  ...
  if (outputPath && outputPath.trim()) {
    resolvedOutputPath = isAbsolutePath(outputPath) ? outputPath : join(baseDir, outputPath);
  } else if (outputFile && outputFile.trim()) {
    resolvedOutputPath = isAbsolutePath(outputFile) ? outputFile : join(baseDir, outputFile);
  }
  ...
  if (typeof maxConsoleLines === "number") out.maxConsoleLines = maxConsoleLines;
  if (typeof includeUntracked === "boolean") out.includeUntracked = includeUntracked;
  if (typeof maxNewFileSizeBytes === "number") out.maxNewFileSizeBytes = maxNewFileSizeBytes;
  ...
  if (typeof includePrTemplate === "boolean") out.includePrTemplate = includePrTemplate;
...
export async function loadUserConfig(baseDir: string): Promise<Partial<Options>> {
  ...
  const candidates = [join(baseDir, "diff2prompt.config.json"), join(baseDir, ".diff2promptrc")];
